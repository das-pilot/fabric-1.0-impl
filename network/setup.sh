...
function printHelp () {
  echo "Usage: "
  echo "  setup.sh -m up|down|restart|generate [-c <channel name>] [-t <timeout>]"
  ...
  echo
  echo "	setup.sh -m generate -c <channelname>"
  echo "	setup.sh -m up -c <channelname>"
  echo "	setup.sh -m down -c <channelname>"
  echo
  echo "Taking all defaults:"
  echo "	setup.sh -m generate"
  echo "	setup.sh -m up"
  echo "	setup.sh -m down"
}
...
function replacePrivateKey () {
  ...
  sed $OPTS "s|THE_GOPATH|${GOPATH}|g" docker-compose-e2e.yaml
...
# (``peer0.org1.das-pilot.com`` & ``peer0.org2.das-pilot.com``).  Secondly, we point to
